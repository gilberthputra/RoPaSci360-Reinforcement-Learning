...
class RoPaSci360(Env):
    def __init__(self, player = 'upper', opponent = 'random', log = True):
        ...
        self.action_space = MultiDiscrete([3, 61, 61])
        self.observation_space = Box(np.int(0), np.int8(-1), shape = (9, 9), dtype = np.int8)
    ...
    def reset(self):
        self.observation = self.new_board()
    ...
    def new_board():
        ...
        for i in range(BOARD_SIZE // 2):
            board[i, i + 5:BOARD_SIZE] = 0
            board[i + 5:BOARD_SIZE, i] = 0
        return Box(np.int(0), np.int8(-1), shape = (9, 9), dtype = np.int8).sample()
    ...
    def step(self, action): 
        ...
        if move_check:
            ...
            return self.observation, INVALID_ACTION_REWARD, self.done, self.info
        ...
        if player_1_valid:
            ...
            self.pieces = pieces
            self.observation = self.insert_to_board(self.pieces)
    ...
    def get_neighbours(token):
        ...
        return [(s, x, y) for (x, y) in nbr if (x, y) in AXIAL_GRID]
    ...
    def insert_to_board(self, pieces):
        board = self.new_board()
        p1 = self.get_tokens(self.player_1, pieces)
        p2 = self.get_tokens(self.player_2, pieces)

        for p1_t in p1:
            (s, r, q) = p1_t
            (x, y) = self.convert_to_normal((r, q))
            board[x][y] = s
        for p2_t in p2:
            (s, r, q) = p2_t
            (x, y) = self.convert_to_normal((r, q))
